  ...
  const toggleStatus = async (id, currentStatus) => {
    ...
    try {
      await fetch('/api/records', {
        ...
        body: JSON.stringify({ module: 'imagem', id, status: newStatus }),
      });
  ...
  return (
    <>
      ...
      <div className="card" style={{ marginBottom: '24px' }}>
        <h2 className="card-title">Registrar Exame</h2>
      ...
      <div className="card">
        ...
        <div className="total-bar"><span className="total-label">Total pago a Volantes Exames de Imagem no mês</span><span className="total-value">{formatCurrency(total)}</span></div>
      </div>
    </>
  );
}
